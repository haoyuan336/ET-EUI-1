...
namespace ET
{
    public sealed class KSService : AService
    {
        ...
        public KSService(AddressFamily addressFamily, NetworkProtocol protocol, ServiceType serviceType)
        {
            this.ServiceType = serviceType;
            this.startTime = TimeInfo.Instance.ClientFrameTime();
            this.Protocol = protocol;
            this.Transport = new UnityWebSocketTransport();
        ...
        public KSService(IKcpTransport transport, NetworkProtocol protocol, ServiceType serviceType)
        {
        ...
        // 保存所有的channel
        private readonly Dictionary<long, KSChannel> localConnChannels = new();
        private readonly Dictionary<long, KSChannel> waitAcceptChannels = new();
        ...
        {
            KSChannel kChannel = this.Get(channelId);
            if (kChannel == null)
            {
                    ...
                    switch (flag)
                    {
                        ...
                        case KcpProtocalType.SYN: // accept
                        {
                            ...
                            if (kChannel == null)
                            {
                                ...
                                kChannel = new KSChannel(localConn, remoteConn, this.ipEndPoint.Clone(), this);
        ...
        {
            Log.Warning($"create channel {id} {ipEndPoint} ");
            if (this.localConnChannels.TryGetValue(id, out KSChannel kChannel))
            {
                return;
            }
            ...
            try
            {
                ...
                kChannel = new KSChannel(localConn, ipEndPoint, this);
                this.localConnChannels.Add(kChannel.LocalConn, kChannel);
            }
            catch (Exception e)
            {
        ...
        public override void Remove(long id, int error = 0)
        {
            ...
            if (this.waitAcceptChannels.TryGetValue(kChannel.RemoteConn, out KSChannel waitChannel))
            {
        ...
        {
            KSChannel channel = this.Get(channelId);
            if (channel == null)
            {
                return;
            }
        ...
        private void CheckWaitAcceptChannel(uint timeNow)
        {
            ...
            {
                if (!this.waitAcceptChannels.TryGetValue(id, out KSChannel kChannel))
                {
                    continue;
                }
        ...
        private void UpdateChannel(uint timeNow)
        {
            ...
            {
                KSChannel kChannel = this.Get(id);
                if (kChannel == null)
                {
                    continue;
                }
