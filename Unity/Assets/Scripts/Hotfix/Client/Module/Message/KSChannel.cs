...
namespace ET
{
	public class KSChannel : AChannel
	{
		private const int MaxKcpMessageSize = 10000;
		
		private readonly KSService Service;
		...
		// connect
		public KSChannel(uint localConn, IPEndPoint remoteEndPoint, KSService kService)
		{
		...
		// accept
		public KSChannel(uint localConn, uint remoteConn, IPEndPoint remoteEndPoint, KSService kService)
		{
			this.Service = kService;
		...
		{
			Log.Debug($"is connected {this.IsConnected} {timeNow}");
			try
			{
				if (this.IsConnected)
				{
