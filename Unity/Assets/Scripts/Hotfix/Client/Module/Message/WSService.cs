...
namespace ET
{
    public class WSService : AService
    {
        private long idGenerater = 200000000;

        private HttpListener httpListener;

        private readonly Dictionary<long, WSChannel> channels = new Dictionary<long, WSChannel>();

        public ThreadSynchronizationContext ThreadSynchronizationContext;

        // public WSService(IEnumerable<string> prefixs)
        // {
        ...
        public override void Create(long id, IPEndPoint ipEndpoint)
        {
            ...
            // WSChannel channel = new(id, webSocket, ipEndpoint, this);
            ...
            Log.Warning($"create url {url}");
            
            UnityWebSocket.WebSocket webSocket = new(url);
        ...
        //
        //                 WSChannel channel = new WSChannel(this.GetId, webSocketContext, this);
        //                 
        ...
        {
            this.channels.TryGetValue(channelId, out WSChannel channel);
            if (channel == null)
            {
                return;
            }
