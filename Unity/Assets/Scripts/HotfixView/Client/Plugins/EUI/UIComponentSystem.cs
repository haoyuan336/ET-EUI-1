...
namespace ET.Client
{
    ...
    public static partial class UIComponentSystem
    {
        ...
        public static void HideWindow(this UIComponent self, WindowID id)
        {
            ...
            baseWindow.GComponent?.RemoveFromParent();
        ...
        public static void UnLoadWindow(this UIComponent self, WindowID id, bool isDispose = true)
        {
            ...
            if (baseWindow.IsPreLoad)
            {
                ...
                // self.Scene().GetComponent<ResourcesLoaderComponent>().UnLoadAssetSync("");

                string packageName = ComponentPackageMap.Get(id);

                UIPackage.RemovePackage(packageName);

                baseWindow.GComponent.Dispose();
            }
        ...
        {
            Log.Debug("ReadyToShowBaseWindow");
            UIBaseWindow baseWindow = self.GetUIBaseWindow(id);
            // 如果UI不存在开始实例化新的窗口
            if (null == baseWindow)
            {
        ...
        private static void LoadBaseWindows(this UIComponent self, UIBaseWindow baseWindow)
        {
            ...
            // string packageName = FGUIComponentPackageMap.Get(baseWindow.FGUIComponentID);

            Log.Debug("load base window");
            string packageName = ComponentPackageMap.Get(baseWindow.WindowID);

            Log.Debug($"package name {packageName}");

            UIPackage.AddPackage($"FGUI/{packageName}/{packageName}");

            GComponent gComponent = UIPackage.CreateObject(packageName, baseWindow.WindowID.ToString()).asCom;

            baseWindow.GComponent = gComponent;
